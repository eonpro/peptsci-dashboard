...
// Get sales data from Google Sheets and transform it
export async function getSales(): Promise<Sale[]> {
  try {
    ...
    for (const row of dataRows) {
      ...
      const invoicePaid = invoicePaidValue.toLowerCase() === 'yes' || 
    ...
    logger.error('Error fetching sales', {}, error instanceof Error ? error : new Error(String(error)))
    return []
  }
}
...
export async function getInventory(): Promise<Inventory[]> {
  try {
    ...
    logger.error('Error fetching inventory', {}, error instanceof Error ? error : new Error(String(error)))
    return []
  }
}

// Get price sheet data from Inventory and Retail Pricing sheets
export async function getPriceSheet(): Promise<PriceSheet[]> {
  try {
    ...
    inventory.forEach(item => {
      ...
      const normalizedProduct = item.MedicationName
      ...
      if (retailRows.length > 1) {
        ...
        for (const row of retailDataRows) {
          ...
          if (product) {
            ...
            const existingIndex = priceSheet.findIndex(item => 
    ...
    logger.error('Error fetching price sheet', {}, error instanceof Error ? error : new Error(String(error)))
    return []
  }
}
...
export async function getCompetitors(): Promise<Competitor[]> {
  try {
    ...
    logger.error('Error fetching competitors', {}, error instanceof Error ? error : new Error(String(error)))
...
function getDefaultCompetitorData(): Competitor[] {
  return [
    { Competitor: 'CompoundingRx', Product: 'Semaglutide', Dose: '10mg', TheirPrice: 450, OurSRP: 399, Diff: -51 },
    { Competitor: 'CompoundingRx', Product: 'Tirzepatide', Dose: '60mg', TheirPrice: 950, OurSRP: 849, Diff: -101 },
    { Competitor: 'PeptideSource', Product: 'Semaglutide', Dose: '10mg', TheirPrice: 425, OurSRP: 399, Diff: -26 },
    { Competitor: 'PeptideSource', Product: 'BPC-157', Dose: '10mg', TheirPrice: 350, OurSRP: 299, Diff: -51 },
    { Competitor: 'PeptideSource', Product: 'GHK-Cu', Dose: '100mg', TheirPrice: 500, OurSRP: 449, Diff: -51 },
    { Competitor: 'MedCompound', Product: 'Semaglutide', Dose: '5mg', TheirPrice: 275, OurSRP: 249, Diff: -26 },
    { Competitor: 'MedCompound', Product: 'Tirzepatide', Dose: '30mg', TheirPrice: 550, OurSRP: 499, Diff: -51 },
    { Competitor: 'MedCompound', Product: 'Retatrutide', Dose: '20mg', TheirPrice: 725, OurSRP: 649, Diff: -76 },
    { Competitor: 'PharmaCo', Product: 'BPC-157', Dose: '10mg', TheirPrice: 375, OurSRP: 299, Diff: -76 },
    { Competitor: 'PharmaCo', Product: 'TB-500', Dose: '10mg', TheirPrice: 325, OurSRP: 279, Diff: -46 },
    { Competitor: 'PeptideLabs', Product: 'Semaglutide', Dose: '10mg', TheirPrice: 475, OurSRP: 399, Diff: -76 },
    { Competitor: 'PeptideLabs', Product: 'NAD+', Dose: '500mg', TheirPrice: 600, OurSRP: 549, Diff: -51 },
  ]
}
