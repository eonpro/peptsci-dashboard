...
export function getTotals(sales: Sale[]): SalesKPIs {
  ...
  const uniqueClients = uniqueEmails.size + Array.from(uniquePhones).filter(
    phone => !Array.from(uniqueEmails).some(email => 
...
export function groupByProduct(sales: Sale[]): ProductMetrics[] {
  ...
  sales.forEach((sale) => {
    ...
    metrics.avgPricePerVial = metrics.totalVials > 0 
      ? metrics.totalRevenue / metrics.totalVials 
      : 0
...
export function groupByCustomer(sales: Sale[]): CustomerMetrics[] {
  ...
  sales.forEach((sale) => {
    ...
    const displayName =
      name ||
      (email ? email.split('@')[0] : phoneDigits ? `Client ${phoneDigits.slice(-4)}` : 'Unknown Customer')
  ...
  customerMap.forEach((metrics) => {
    ...
    metrics.avgOrderValue = metrics.totalOrders > 0 
      ? metrics.lifetimeSpend / metrics.totalOrders 
      : 0
  })
...
export function getCustomerById(sales: Sale[], id: string): CustomerMetrics | null {
  ...
  return customers.find(c => {
    const slug = c.email?.toLowerCase() || 
                 `${c.phone}_${c.name}`.toLowerCase().replace(/[^a-z0-9_]/g, '-')
