...
export const authAppearance = {
  ...
  elements: {
    ...
    // Phone field: the ring lives on the wrapper (country selector + input) so
    // the whole control reads as one piece. The inner input is flattened via
    // the `.cl-phoneInputBox` rules in globals.css (appearance classes merge
    // with the generic formFieldInput ones and the conflict resolution is
    // stylesheet-order-dependent, so plain CSS is the deterministic path).
    phoneInputBox:
