...
type Line = { variantId: string; label: string; quantity: number; unitPrice: number; available: number }
...
export default function ConvertStripeModal({ open, onOpenChange, record, onConverted }: ConvertStripeModalProps) {
  ...
  const addLine = (v: VariantRow) => {
    setLines((prev) => (prev.some((l) => l.variantId === v.id) ? prev : [...prev, { variantId: v.id, label: `${v.productName}${v.dose ? ` ${v.dose}` : ''}${v.sku ? ` · ${v.sku}` : ''}`, quantity: 1, unitPrice: v.srp, available: v.available }]))
    setProductQuery('')
  }
  const updateLine = (id: string, patch: Partial<Line>) => setLines((prev) => prev.map((l) => (l.variantId === id ? { ...l, ...patch } : l)))
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
        ...
        <div className="mb-1 rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm">
          <div className="flex justify-between"><span className="text-blue-700">{record.customerName || record.customerEmail || 'Unknown customer'}</span><span className="font-semibold text-blue-900">{formatPrice(record.paidAmount)}</span></div>
          <p className="mt-1 text-xs text-blue-600">{record.orderRef} · {record.product || 'No line detail'}{record.vials ? ` · ${record.vials} vials` : ''}</p>
        </div>

        {loadingRefs ? (
          <div className="flex items-center justify-center py-10 text-gray-500"><Loader2 className="mr-2 h-5 w-5 animate-spin" /> Loading catalog…</div>
        ) : (
          <div className="space-y-6">
              ...
              {selectedClient ? (
                <div className="flex items-center justify-between rounded-lg border border-gray-200 p-3">
                  <div>
                    ...
                    <p className="text-xs text-gray-500">{[selectedClient.contactName, selectedClient.contactEmail].filter(Boolean).join(' · ') || record.customerEmail || '—'}</p>
                  </div>
                    ...
                    <div className="max-h-44 overflow-y-auto rounded-lg border border-gray-200">
                      {filteredClients.length === 0 ? (
                        ...
                        filteredClients.map((c) => (
                          <button key={c.id} type="button" onClick={() => { setClientId(c.id); setClientQuery('') }} className="flex w-full flex-col items-start px-3 py-2 text-left hover:bg-gray-50">
            ...
            {/* Products */}
            <fieldset className="space-y-2">
              <legend className="text-sm font-semibold text-gray-700">Map products</legend>
              <div className="relative">
                <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" />
                ...
                {filteredVariants.length > 0 && (
                  <div className="absolute z-10 mt-1 max-h-52 w-full overflow-y-auto rounded-lg border border-gray-200 bg-white shadow-lg">
                    {filteredVariants.map((v) => (
                      <button key={v.id} type="button" onClick={() => addLine(v)} className="flex w-full items-center justify-between px-3 py-2 text-left hover:bg-gray-50">
                        <span className="text-sm text-gray-800">{v.productName}{v.dose ? ` ${v.dose}` : ''}<span className="ml-1 text-xs text-gray-400">{v.sku}</span></span>
                  ...
                  {lines.map((l) => (
                    <div key={l.variantId} className="flex items-center gap-2 p-2">
                      <div className="min-w-0 flex-1">
                        ...
                        <p className={`text-xs ${l.quantity > l.available ? 'text-amber-600' : 'text-gray-400'}`}>{l.quantity > l.available ? `Only ${l.available} in stock (oversell)` : `${l.available} available`}</p>
                      </div>
                      ...
                      <button type="button" onClick={() => removeLine(l.variantId)} className="text-gray-400 hover:text-red-500" aria-label="Remove line"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className="space-y-1 rounded-lg border border-gray-200 bg-gray-50 p-3 text-sm">
              <div className="flex justify-between text-gray-600"><span>Order total (fulfillment)</span><span>{formatPrice(totals.total)}</span></div>
              <div className="flex justify-between text-gray-500"><span>Stripe captured</span><span>{formatPrice(record.paidAmount)}</span></div>
            </div>

            <div className="flex items-center justify-end gap-3 border-t pt-4">
              <Button variant="outline" onClick={() => onOpenChange(false)} disabled={submitting}>Cancel</Button>
              <Button onClick={handleSubmit} disabled={!clientId || lines.length === 0 || submitting} style={{ backgroundColor: ACCENT }}>
