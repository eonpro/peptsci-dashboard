  ...
  if (
    ...
    return (
      <div className="rounded-lg border bg-white p-3 shadow-lg">
        ...
        <p className="text-sm">
          <span className="text-brand-primary">Our Price:</span> ${payload[0].value.toFixed(2)}
        </p>
        <p className="text-sm">
          <span className="text-gray-600">Their Price:</span> ${payload[1].value.toFixed(2)}
        </p>
        {savings > 0 && (
        ...
        {diff === 0 && <p className="text-sm font-medium text-gray-600 mt-1">Same price</p>}
      </div>
    )
  }
  return null
  ...
  return (
    <ResponsiveContainer width="100%" height={400}>
      <BarChart data={data} margin={{ top: 20, right: 30, left: 20, bottom: 80 }}>
        ...
        <Legend />
        <Bar dataKey="ourPrice" fill="#213cef" name="Our Price" radius={[8, 8, 0, 0]} />
        <Bar dataKey="theirPrice" fill="#94a3b8" name="Competitor Price" radius={[8, 8, 0, 0]} />
      </BarChart>
    </ResponsiveContainer>
  )
}
