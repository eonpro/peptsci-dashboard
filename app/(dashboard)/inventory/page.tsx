...
export default function InventoryPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      const res = await fetch(`/api/admin/inventory/batches?status=ALL&t=${Date.now()}`, {
        cache: 'no-store',
      })
  ...
  return (
    <div className="container mx-auto space-y-6 p-6">
      ...
      <ReceiveInventoryModal open={modalOpen} onOpenChange={setModalOpen} onReceived={loadData} />
    </div>
  )
}
