...
type RawDistributorOrder = Omit<DistributorOrder, 'orderDate'> & { orderDate: string | null }
  ...
  for (let monthOffset = 0; monthOffset < 6; monthOffset++) {
    ...
    for (let i = 0; i < numSales; i++) {
      ...
      sales.push({
        ...
        OrderID: `P-${format(month, 'MM')}${String(day).padStart(2, '0')}-${String(
          i + 1
        ).padStart(3, '0')}`,
...
type RawSale = Omit<Sale, 'Date'> & { Date: string | null }
      ...
      try {
        ...
        if (salesRes.ok) {
          ...
          if (Array.isArray(salesJson)) {
            const parsedSales: Sale[] = salesJson.map((raw) => {
              const sale = raw as RawSale
              return {
                ...sale,
        ...
        if (ordersRes.ok) {
          ...
          if (Array.isArray(ordersJson)) {
            const parsedOrders: DistributorOrder[] = ordersJson.map((order) => {
              const raw = order as RawDistributorOrder
              return {
                ...raw,
      ...
      <div className="container mx-auto space-y-6 p-6">
        <div className="rounded-lg border bg-white p-12 text-center shadow-sm">
          ...
          <p className="text-muted-foreground">
            We haven&apos;t received any paid orders yet. Once invoices are marked paid,
            monthly and year-to-date performance will appear here automatically.
          </p>
        </div>
      </div>
    )
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {balanceSheet.inventory.items.slice(0, 10).map((item, index) => (
                  <tr key={`${item.sku || 'item'}-${item.dose}-${index}`} className="border-b last:border-0">
