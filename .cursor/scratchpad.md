...
- [x] **Prod data (Jul 21 ~12:45 AM ET)**: via the owner's signed-in admin session — `POST /api/admin/sales/backfill-orders` (1/1 synced) + `POST /api/admin/sales/backfill-stripe` full pass (467 scanned = entire PI history, 373 updated, 0 failed; first attempt died only because the browser tab was navigated mid-request — chunk with `maxScan` and lock the tab). Dashboard verified: Top Products now shows real products (Tirzepatide 60mg $132,290 / Sema 10mg / Sema 5mg...), no "+N more" buckets; all-time revenue unchanged at $510,451.
- Data-quality observations for owner (not bugs): "Kangaroo Mailers" (packaging) now surfaces as a top "product" by vial count, and "Ret20" vs "Reta 20mg" are inconsistent Stripe line descriptions that bucket separately — consider a product-name normalization map if these should merge/be excluded.
...
## World-class affiliate platform — waves 1-4 (Jul 19 2026) [EXECUTOR]

**Shipped + deployed + prod migrations applied (`0a3e634`; migrations `20260720031000_partner_platform_wave1_4` + `20260720033000_partner_txn_optional_client`). Verified: tsc/lint clean, 333/333 tests, build green, extended DB smoke (11 scenarios incl. lead email/NPI matching + tier rate bumps), prod probes all true, live surface checks pass.**

- **W1 acquisition:** `PartnerLead` protected leads (90d window; onboarding matches email/NPI when no cookie — cookie wins; auto-CONVERT + partner email; admin manual match `PATCH /api/admin/partners/[id]/leads`); `ReferralLinkClick` events (daily-salted visitor hash, UTM capture) + 30-day timeseries/top-sources in links page; co-branded `/p/<code>` invitation landing (CTA routes through /join for cookie+click); QR PNG endpoint (`qrcode` dep) + UTM builder buttons.
- **W2 transparency:** partner emails (clinic attributed, payout recorded, daily digest — org `notifyByEmail` pref); `/partners/statements` (true running balances incl. pre-window activity) + CSV dataset; acquisition-funnel block + anonymized quarterly leaderboard on portal home; `/partners/terms` (rates, tiers, hold, payout policy, W-9, MSA, partner-referral link, notification toggle).
- **W3 payout ops:** `partner-daily` cron (12:30 UTC; auto-approve past per-org `holdDays` when `autoApproveEntries`, expire stale leads, send digests); org `payoutMinimumCents` + W-9 upload (blob-only, `POST /api/partners/settings`); `PartnerPayoutRequest` flow (portal button w/ gates → admin bell → auto-COMPLETED when payout recorded); admin program-wide payout queue (`GET /api/admin/partners/payout-queue`) on /partners-admin.
- **W4 growth:** org `teamJoinCode` self-serve rep applications (`/partners/join-team/<code>` public page → PENDING rep → org "Approve & invite" sends Clerk invite); `PartnerAsset` marketing library (admin `/partners-admin/assets` publish → portal `/partners/assets`); `PartnerRateTier` quarterly volume bonuses applied in accrual via `effectiveOrgRateBps` (capped 100%, reps clamped to boosted rate); partner-refers-partner (`partnerRefCode` → apply `?pref=` → `referredByOrgId` → admin one-time bonus as clinic-less MANUAL transaction — `PartnerTransaction.clientId` now optional; all consumers null-guarded as "Program bonus").
- Lessons: Next streams dynamic pages with a loading boundary, so `notFound()` bodies can arrive with HTTP 200 — check rendered content, not status, when smoke-testing 404s. `prisma migrate dev` is interactive-only under the agent shell; use `migrate diff --from-config-datasource … --script` + `migrate deploy` instead (local Docker PG must be started first).

## Clinic-to-clinic referrals — 5% store credit (Jul 18 2026) [EXECUTOR]
...
- Checkout: `applyCredit` flag → `createDraftOrder` clamps to the REAL balance inside the per-client advisory lock (parallel open drafts hold their credit; Stripe $0.50 minimum respected; order.total stored net so the PI amount-match guard holds). Fully-covered orders skip Stripe: explicit "Place Order — $0 due" button → SUBMITTED+CAPTURED, redemption row, reservation, SalesRecord, confirmation email. Terms checkout unchanged (credit is card-path only).
- UI: Shop → "Refer & Earn" (`/shop/referrals`): link+copy, balance, referred clinics w/ per-clinic earnings, ledger history; checkout credit toggle w/ due preview; admin `ClientCreditCard` on `/clients/[id]` (balance, history, manual ± adjustment w/ required note, floor-at-zero).
- **Verified:** tsc/lint clean, 320/320 tests, `next build` green; DB smoke (`scripts/smoke-referral-credit.ts`): earn→idempotent→50% clawback→clamp→parallel-draft hold→redeem→full-refund restore→Stripe-minimum, all passed; prod migration probes true; prod surfaces respond correctly (bad /refer code → no cookie; APIs auth-gated).
- Lesson: Next 15 route handlers reject non-handler exports (`export const CLINIC_REF_COOKIE` in route.ts fails `next build` type check) — constants live in the lib module.
...
### Follow-ups activated (Jul 20 ~23:00 EDT)
- `PARTNER_STRIPE_PAYOUTS_ENABLED=true` set in Vercel prod + redeployed (Ready). Partner orgs can now onboard to Stripe Express from /partners/payouts; admin "Pay org via Stripe" is live. ⚠️ Platform Stripe balance must be funded before real transfers succeed (deterministic failures unwind the ledger safely).
- CI e2e secrets set (`gh secret set E2E_CLERK_PUBLISHABLE_KEY/SECRET_KEY`). NOTE: no Clerk dev/test instance exists anywhere (local + preview + prod are all pk_live/sk_live), so the LIVE keys were used — smoke suite is read-only so this is acceptable, but creating a Clerk dev instance and rotating the CI secrets to pk_test is the right long-term move.
- First real e2e CI run surfaced 2 smoke-spec bugs, fixed in `9244796`: the privacy STOP locator crossed an element boundary (bold **STOP** is its own node → `getByText('STOP', {exact:true})`), and the sign-in widget can't render on localhost with domain-locked live keys (test now skips only in that combination; still runs against deployed URLs). Suite verified 6/6 against prod, then CI run 29797425538 fully green (quality + e2e-smoke both success).
