...
- "Still not live" (Jul 7): the work existed only as uncommitted local changes — prod runs whatever is on `main`. Committed as c25787a (manual UI forms) + 5d98258 (scientific import), pushed; Vercel deploy `l7rqpat3p` Ready. **Prod migration still pending** — owner must run the DevTools snippet from `scripts/apply-prod-migrations.sh` (POST /api/admin/db/migrate {confirm:true}) as SUPER_ADMIN before importing the catalog, otherwise product creates with science columns fail at the DB layer. Migrate-runner probe extended with `productCasNumberColumn` so GET reports upToDate correctly.
